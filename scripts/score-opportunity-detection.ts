...
const [engine, route, component, contract, workflow, docs] = await Promise.all([
...
const checks = [
  ...
  ["dark-sky truth boundary", /do not claim aurora|does not claim aurora/i.test(docs)],
  ...
  ["branded/non-branded brief", /Non-branded/.test(await read("scripts/build-growth-brief.ts"))],
